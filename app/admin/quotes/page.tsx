...
export default function AdminQuotesPage() {
  ...
  const handleDelete = async (clientId: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setQuotes((prev) => prev.filter((q) => q.Client_ID !== clientId));
      } else {
                    ...
                    <tr key={quote.Client_ID} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="p-4 text-right">
                        <div className="inline-flex items-center gap-1">
                          ...
                          <button
                            onClick={() => handleDelete(quote.Client_ID)}
                            disabled={deletingId === quote.Client_ID}
                            className="p-2 text-muted-foreground hover:text-red-600 hover:bg-red-50 rounded transition-colors disabled:opacity-40"
                            title="Delete"
                          >
                            <DeleteIcon />
                          </button>

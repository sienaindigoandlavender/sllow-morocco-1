...
const LEGAL_PAGES: Record<string, { title: string; sections: { order: number; title: string; content: string }[] }> = {
  ...
  "cancellations-and-refunds": {
    ...
    sections: [
      { order: 1, title: "Cancellation by You", content: "We understand plans change. If you need to cancel your booking, the following refund schedule applies based on when we receive written notice (email):\n\nMore than 45 days before departure — deposit retained, balance refunded in full.\nBetween 30 and 45 days before departure — deposit retained, balance refunded minus any non-refundable deposits already committed to properties and suppliers on your behalf. We will provide a full accounting.\nLess than 30 days before departure — no refund. All commitments are locked at this stage.\n\nThe date we receive your written notice determines the refund tier." },

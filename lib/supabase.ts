...
export async function getPlaces(options?: {
  ...
  let query = supabase.from("places").select("slug,title,destination,category,address,opening_hours,fees,notes,hero_image,hero_caption,excerpt,tags,journey_bridge,published,featured,sort_order,created_at,updated_at,schema_type,related_story_slugs,latitude,longitude,visit_duration_minutes,best_time_to_visit,getting_there,nearby_slugs,faq_data");
  if (options?.published !== undefined) query = query.eq("published", options.published);
...
export async function getStories(options?: {
  ...
  const LISTING_FIELDS = "slug,title,subtitle,category,source_type,hero_image,hero_caption,excerpt,read_time,year,text_by,images_by,tags,published,featured,sort_order,region,country,theme,era,era_start,era_end,embed_url,journey_bridge,updated_at,created_at";
